...
sparse sparse_general_product_(sparse a, sparse b, map m, project_map pm, dense_grade_map dgm, float precision){
    ...
    for(int i = 0; i < a_size; i++){
        ...
        for(int j = 0; j < b_size; j++){
            ...
            if(sign == 0)
                continue;
            size_t bitmap = m.bitmap[a.bitmap[i]][b.bitmap[j]];
...
sparse sparse_add_add_(sparse a, sparse b, size_t size, float precision){
...
sparse sparse_dense_to_sparse_sparse(sparse dense_y, size_t size){
    size_t k = 0;
...
void sparse_remove_small(sparse y, float precision,size_t *size){
...
sparse initialize_sparse(size_t size){
    sparse y;
