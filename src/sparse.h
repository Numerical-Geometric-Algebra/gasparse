...
sparse initialize_sparse(size_t);
int comp_abs(float,float);
sparse sparse_product(sparse_multivectors);
sparse sparse_product_(sparse,sparse,map,float);
sparse sparse_grade_project(sparse,size_t*,size_t,dense_grade_map);
sparse sparse_dense_to_sparse_sparse(sparse,size_t);
void sparse_remove_small(sparse,float,size_t*);
...
sparse sparse_add_add_(sparse,sparse,size_t,float);
