...
/* Specialized Ternary product for general graded product operations*/
static int specialized_ternary_blades_product0(void *out, void *data0, void *data1, void *data2, int *grades_out, Py_ssize_t gsize, PyAlgebraObject *ga, ProductType ptype1, ProductType ptype2){
    ...
    if(blades0->size == 1 && blades1->size == 1 && blades2->size == 1 && gsize == 1){
        ...
        PositionMap tmap = ga->ternary_product[ptype1][ptype2][*blades0->grade][*blades1->grade][*blades2->grade];
        ...
        for(Py_ssize_t i0 = 0; i0 < sparse0.size; i0++){
            ...
            for(Py_ssize_t i1 = 0; i1 < sparse1.size; i1++){
                ...
                for(Py_ssize_t i2 = 0; i2 < sparse2.size; i2++){
                    ...
                    sparse.value[tmap2.position] = tmap2.sign*value0*sparse2.value[i2];
                }
            }
        }
    ...
    for(Py_ssize_t j0 = 0; j0 < blades0->size; j0++){
        ...
        for(Py_ssize_t j1 = 0; j1 < blades1->size; j1++){
            ...
            for(Py_ssize_t j2 = 0; j2 < blades2->size; j2++){
                ...
                PositionMap tmap = ga->ternary_product[ptype1][ptype2][blades0->grade[j0]][blades1->grade[j1]][blades2->grade[j2]];
                for(Py_ssize_t i0 = 0; i0 < sparse0.size; i0++){
                    Py_ssize_t position0 = gm.position[sparse0.bitmap[i0]];
                    MapBase **tmap0 = tmap[position0];
                    for(Py_ssize_t i1 = 0; i1 < sparse1.size; i1++){
                        ...
                        for(Py_ssize_t i2 = 0; i2 < sparse2.size; i2++){
                            ...
                            sparse[tmap2.grade].value[tmap2.position] = tmap2.sign*value0*sparse2.value[i2];
                        }
                    }
                }
            }
...
static int specialized_ternary_gradeddense_product0(void *out, void *data0, void *data1, void *data2, int *grades_out, Py_ssize_t gsize, PyAlgebraObject *ga, ProductType ptype1, ProductType ptype2){
    ...
    GradeTable gt = ga->gt;
    Py_ssize_t *g = get_grade_bool(grades_out, gsize, gm.max_grade + 1);
    SparseTernaryMap map = ga->sparse_ternary_product[ptype1][ptype2];
    ...
    if(gsize == 1 && mv0->size == 1 && mv1->size == 1 && mv2->size == 1){
        ...
        *dense.grades = *grades_out;
        ...
        for(Py_ssize_t j4 = 0; j4 < smap.size; j4++){
            ...
            values_out[base.position[3]] += values0[base.position[0]]*values1[base.position[1]]*values2[base.position[2]]*base.sign;
        }
        *mv = dense;
        return 1;
    }
