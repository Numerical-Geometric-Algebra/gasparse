...
static int algebra_init(PyAlgebraObject *self, PyObject *args, PyObject *kwds) {
  ...
  case ComputationMode_generated:;
    ...
    // set default type
    self->mdefault.type_name =
        (char *)PyMem_RawMalloc((strlen("dense") + 1) * sizeof(char));
  ...
  case ComputationMode_large:;
    ...
    // set default type
    self->mdefault.type_name =
        (char *)PyMem_RawMalloc((strlen("sparse") + 1) * sizeof(char));
...
PyMultivectorObject *populate_multivector_types(PyAlgebraObject *self, char *dtype_str) {
  ...
  multivector =
      (PyMultivectorObject *)PyMem_RawMalloc(sizeof(PyMultivectorObject));
...
static Py_ssize_t parse_list_as_values(PyObject *values,
...
static PyObject *algebra_set_multivector_defaults(PyAlgebraObject *self,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|f", kwlist, &dtype,
                                   &precision))
    return NULL;
  ...
  self->mdefault.type_name =
      (char *)PyMem_RawMalloc((strlen(dtype) + 1) * sizeof(char));
...
static PyObject *algebra_multivector(PyAlgebraObject *self, PyObject *args,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|OO", kwlist, &values, &basis,
                                   &grades))
    return NULL;
  ...
  multivector = populate_multivector_types(self,self->mdefault.type_name);
  if (!multivector) {
    PyMem_RawFree(values_float);
    PyMem_RawFree(bitmaps_int);
    return NULL;
...
int get_multivector_basis(PyAlgebraObject *self, PyObject *grades, PyMultivectorObject ***multivectors, Py_ssize_t *mvsize){
    ...
    *multivectors = (PyMultivectorObject **)PyMem_RawMalloc(
      size * sizeof(PyMultivectorObject *));
    if (!multivectors)
        goto fail;
    for (Py_ssize_t i = 0; i < size; i++)
        (*multivectors)[i] = NULL;
    for (Py_ssize_t i = 0; i < size; i++) {
        (*multivectors)[i] = populate_multivector_types(self,self->mdefault.type_name);
        if (!(*multivectors)[i])
        goto fail;
    }
    *mvsize = size;
...
static PyObject *algebra_blades(PyAlgebraObject *self, PyObject *args,
                                PyObject *kwds) {
